package com.marketteam.desarrollo.proof;
...
public class ActualizarTablas {
    ...
    public void insertarT1(String idCliente, ArrayList<String> datosAnt, ArrayList<String> datosNuev, double[] coord) {
        //Guardar datos (nuevos) en Tabla1
        String deviceName = Build.ID;
        String IDAud = IdAuditoria();
        String queryCli = "SELECT va FROM PA WHERE pa=?";
        ArrayList<String>[] objeto = conGen.queryObjeto(context, queryCli, new String[]{"user"});
        String user = "";
        if (objeto != null) {
            user = objeto[0].get(0);
        }
        ...
        cv.put("maquina", deviceName);
        cv.put("fecha_ini", fechaC);
        cv.put("hora_ini", horaC);
        latitudeGPS = coord[0];
        longitudeGPS = coord[1];
        cv.put("gpslat_ini", "" + latitudeGPS);
        System.out.println("Latitud ---> " + latitudeGPS);
        cv.put("gpslon_ini", "" + longitudeGPS);
        System.out.println("Longitud ---> " + longitudeGPS);
        cv.put("usuario", user);
        cv.put("estado", 0); //Estado: En curso
        //Valores de la actividad a t1
        cv.put("cliente_t", idCliente);
        cv.put("cliente_n", idCliente);
        cv.put("maestro1", datosAnt.get(0));
        cv.put("maestro2", datosAnt.get(1));
        cv.put("maestro3", datosAnt.get(2));
        cv.put("maestro4", datosAnt.get(3));
        cv.put("maestro5", datosAnt.get(4));
        cv.put("maestro6", datosAnt.get(5));
        cv.put("maestro7", datosAnt.get(6));
        cv.put("maestro8", datosAnt.get(7));
        cv.put("maestro9", datosAnt.get(8));
        cv.put("mconfir1", datosNuev.get(0));
        cv.put("mconfir2", datosNuev.get(1));
        cv.put("mconfir3", datosNuev.get(2));
        cv.put("mconfir4", datosNuev.get(3));
        cv.put("mconfir5", datosNuev.get(4));
        cv.put("mconfir6", datosNuev.get(5));
        cv.put("mconfir7", datosNuev.get(6));
        cv.put("mconfir8", datosNuev.get(7));
        cv.put("mconfir9", datosNuev.get(8));
        cv.put("sincronizada", "0");
        cv.put("fcr", fg.fechaActual(3));
        boolean insert = operaciones.insertar("t1t", cv);
        if (insert != true) {
            Toast.makeText(context, "No se ha podido insertar la info en la tabla", Toast.LENGTH_SHORT).show();
        }
        Toast.makeText(context, "Las tablas están siendo cargadas, esto tardará entre 6 y 12 segundos", Toast.LENGTH_LONG).show();
        String ciudad = datosAnt.get(0);
        String canal = datosAnt.get(7);
        //Guardar ciudad , auditoría(ID), canal y subcanal en ACT
        ...
        operaciones.queryNoData("UPDATE ACT SET VAL='" + IDAud + "' WHERE VA='AUD'");
        //Traer el id de 201_CIU_CAN where CIU=ciudad AND CAN=canal
        ArrayList<String>[] idc = conGen.queryGeneral(context, "'201_CIU_CAN'", new String[]{"_ID"}, "CIU='" + ciudad + "' AND CAN='" + canal + "'");
        if (idc != null) {
            String id = idc[0].get(0);
            //Guardar el resultado en ACT - VA=CIUCAN -> VAL=resultado
